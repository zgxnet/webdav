...
public class LockHandler : IRequestHandler
{
    ...
    public async Task<bool> HandleRequestAsync(HttpContext httpContext)
    {
        ...
        var item = await _store.GetItemAsync(requestPath, httpContext.RequestAborted).ConfigureAwait(false);
        if (item == null)
        {
        ...
        else
        {
            ...
            // Build URI for lock root
            var lockRootUri = request.GetUri();

            // Perform the lock
            lockResult = await _lockingManager.LockAsync(item, lockType, lockScope, owner, lockRootUri, depth > 0, timeouts, httpContext.RequestAborted).ConfigureAwait(false);
        }
