...
public class PropFindHandler : IRequestHandler
{
    ...
    private readonly record struct PropertyEntry(string Path, IStoreItem Entry);
    ...
    public async Task<bool> HandleRequestAsync(HttpContext httpContext)
    {
        ...
        // Obtain entry using request path
        var requestPath = request.Path.Value ?? "/";
        var topEntry = await _store.GetItemAsync(requestPath, httpContext.RequestAborted).ConfigureAwait(false);
        if (topEntry == null)
        {
        ...
        // Build base URI for response hrefs
        ...
        if (topEntry is IStoreCollection topCollection)
        {
            ...
            // Add all the entries
            await AddEntriesAsync(topCollection, depth, requestPath, entries, httpContext.RequestAborted).ConfigureAwait(false);
        }
        else
        {
            // It should be an item, so just use this item
            entries.Add(new PropertyEntry(requestPath, topEntry));
        }
        ...
        {
            // Create the property - convert path to URI for href
            // Combine PathBase (e.g., /dav) with the entry path to get the full path
            var fullPath = request.PathBase.Add(entry.Path).Value ?? entry.Path;
            var entryUri = UriHelper.CombineWithPath(baseUri, fullPath);
            var xResponse = new XElement(WebDavNamespaces.DavNs + "response",
                new XElement(WebDavNamespaces.DavNs + "href", UriHelper.ToEncodedString(entryUri)));
    ...
    private async Task AddEntriesAsync(IStoreCollection collection, int depth, string path, IList<PropertyEntry> entries, CancellationToken cancellationToken)
    {
        // Add the collection to the list
        entries.Add(new PropertyEntry(path, collection));
        ...
        if (depth > 0)
        {
            ...
            await foreach (var childEntry in collection.GetItemsAsync(cancellationToken).ConfigureAwait(false))
            {
                ...
                var childPath = path.TrimEnd('/') + "/" + childEntry.Name;
                if (childEntry is IStoreCollection subCollection)
                    await AddEntriesAsync(subCollection, depth - 1, childPath, entries, cancellationToken).ConfigureAwait(false);
                else
                    entries.Add(new PropertyEntry(childPath, childEntry));
            }
        }
    }
}
