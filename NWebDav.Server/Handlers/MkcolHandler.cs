...
public class MkcolHandler : IRequestHandler
{
    ...
    public async Task<bool> HandleRequestAsync(HttpContext httpContext)
    {
        ...
        // Determine parent path and collection name
        var lastSlash = requestPath.TrimEnd('/').LastIndexOf('/');
        var parentPath = lastSlash > 0 ? requestPath.Substring(0, lastSlash) : "/";
        var collectionName = lastSlash >= 0 ? requestPath.Substring(lastSlash + 1).TrimEnd('/') : requestPath.TrimStart('/');
        ...
        var result = await collection.CreateCollectionAsync(collectionName, false, httpContext.RequestAborted).ConfigureAwait(false);
