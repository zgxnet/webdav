...
public class DeleteHandler : IRequestHandler
{
    ...
    public async Task<bool> HandleRequestAsync(HttpContext httpContext)
    {
        ...
        var itemName = lastSlash >= 0 ? requestPath.Substring(lastSlash + 1).TrimEnd('/') : requestPath.TrimStart('/');

        // Obtain parent collection
        var parentCollection = await _store.GetCollectionAsync(parentPath, httpContext.RequestAborted).ConfigureAwait(false);
        ...
        var deleteItem = await parentCollection.GetItemAsync(itemName, httpContext.RequestAborted).ConfigureAwait(false);
        if (deleteItem == null)
        {
            // Source not found
            response.SetStatus(DavStatusCode.NotFound);
        ...
        // Delete item
        var status = await DeleteItemAsync(parentCollection, itemName, parentPath, httpContext.RequestAborted).ConfigureAwait(false);
    ...
    private async Task<DavStatusCode> DeleteItemAsync(IStoreCollection collection, string name, string basePath, CancellationToken cancellationToken)
    {
        ...
        {
            // Determine the new base path
            var subBasePath = basePath.TrimEnd('/') + "/" + name;

            // Delete all entries first
            await foreach (var entry in deleteCollection.GetItemsAsync(cancellationToken).ConfigureAwait(false))
                await DeleteItemAsync(deleteCollection, entry.Name, subBasePath, cancellationToken).ConfigureAwait(false);
        }
